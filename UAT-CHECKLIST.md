...
- **Two message sources, not one:** most player-facing text in `## Register`, `## Login`,
  ...
  and DO carry `language: en` as a precondition, quoting `lang/en.json`'s text; the one row under
  `## Lifecycle Hooks` also carries it, but only for two framework-owned console lines. A third group —
...
| ultilogin.register.create.neg-already-registered | `session-enabled: false`; the same player as `ultilogin.register.create`, already registered, then quit and rejoined so `RegisterCommand#register`'s own `isLoggedIn` check (which runs BEFORE `isRegistered`) sees them as unauthenticated rather than short-circuiting to `messages.already-logged` | Run `/register newpass1 newpass1` | `messages.already-registered`'s shipped (Simplified Chinese, not reproduced per D-02) text | server | |
...
| ultilogin.changepassword.update.neg-not-logged-in | A player who is not currently logged in; `allowed-commands` includes `changepassword` (NOT the shipped default, which omits it — `LoginProtectionListener#onPlayerCommand` would otherwise cancel this command before it ever reaches `ChangePasswordCommand`, showing the ordinary credential prompt instead of this row's own message) | Run `/changepassword a b b` | A hardcoded Simplified Chinese line (`ChangePasswordCommand.java:43`, not reproduced per D-02), red — a DIFFERENT literal than `EmailBindCommand`'s i18n-routed `please_login_first` ("Please login first!") for the same underlying condition, despite `lang/en.json` declaring that exact key | server | |
...
| ultilogin.admin.info | A registered target player with a bound, verified email | Run `/logadmin info <target>` | Ten hardcoded Simplified Chinese lines — one title line plus nine data fields (`LoginAdminCommand.java:181-197`, not reproduced per D-02: player name, UUID, register/last IP, login count, email + verified state, register/last-login timestamps) — unaffected by `language: en`; `lang/en.json`'s 11 `info_*` keys are declared but never read (`UltiKits/UltiLogin#20`) | server | |
...
| ultilogin.protection.inventory-block | Not logged in; `gui-mode.enabled: false` (so no credential GUI is open); a chest within reach holding at least one item; at least two distinguishable stacks in the player's own inventory, in known slots | Both branches, not either: (1) right-click the chest; (2) open the player's own inventory and drag one of its stacks onto another slot inside it. Read the player's inventory NBT (`data get entity <player> Inventory`) before and after step 2 | (1) no container window is sent to the client — the chest does not open; (2) after the drag, every stack named in the before-reading is still in the same slot with the same count. The two branches are both required because they exercise different guards: a chest is refused when the server opens it, while the player's own inventory is already visible, so its clicks and drags have to be refused one at a time. Phase 10's run of this row exercised only branch (1), which is why branch (2) is now spelled out separately (UltiKits/UltiLogin#24's sweep) | server | |
...
| ultilogin.protection.world-interaction-block | Not logged in; exactly 5 of one stackable block in the selected hotbar slot and the off hand empty; an armor stand summoned within reach with `ShowArms:1b` and a known tag; a breakable block, a lever and a dropped item stack all within reach; a writable book and a placeable sign in the player's inventory | Run a non-whitelisted command (for example `/help`) as a before-control, then attempt, in turn: break the block, place a block, flip the lever, right-click a living entity, right-click the armor stand's arm while holding the block stack (the armor-stand equip path — a plain block is accepted only into a hand slot, which is why the stand needs `ShowArms:1b`), press the off-hand swap key, write and sign the book, place and write the sign, drop the held stack, walk over the dropped item; then run the same command again as an after-control. Read `data get entity <player> SelectedItem`, `data get entity <player> Inventory`, and the armor stand's full entity data before and after | Neither control command is accepted, which proves the player stayed unauthenticated for the whole sequence. None of the ten actions takes effect: the target block is still present and no new block appears, the lever keeps its previous state, `SelectedItem` still reads count 5 (in the reported defect it fell to 4), the armor stand's entity data shows no item in any armor or hand slot, the off hand is still empty and the selected slot unchanged, no dropped-item entity appears for the drop, and the item already on the ground is still an entity afterwards. The book is still unsigned with no text stored and no sign block gains text — note these last two are expected to be refused earlier in the chain (the sign cannot be placed, the book editor should not open), so a pass here does not attribute which guard refused them; `LoginProtectionEventCoverageTest` is what proves the two handlers themselves cancel. A player who does manage to sign a book or write a sign is a failure either way, which is why the steps stay | server | |
...
| ultilogin.gui.login-page.neg-wrong-password | Same as `ultilogin.gui.login-page`; zero prior failed attempts this lockout window (so this attempt lands on the attempts-remaining branch, not the lockout branch) | Enter four wrong digits | The password display resets to the space-separated `○ ○ ○ ○`; the player receives `messages.attempts-remaining`'s shipped text with `{COUNT}` substituted to one less than `security.max-login-attempts` — `LoginGUIPage#attemptLogin` forwards `LoginService#login`'s result unchanged, which is always `messages.attempts-remaining` or, at lockout, `messages.account-locked`; `messages.wrong-password` is never reachable here either (`UltiKits/UltiLogin#23`); the GUI itself remains open, not closed | pixel | LoginGUIPage |
