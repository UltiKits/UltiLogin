...
| ultilogin.protection.world-interaction-block | Not logged in; exactly 5 of one stackable block in the selected hotbar slot and the off hand empty; an armor stand summoned within reach with `ShowArms:1b` and a known tag; a breakable block, a lever and a dropped item stack all within reach | Run a non-whitelisted command (for example `/help`) as a before-control, then attempt, in turn: break the block, place a block, flip the lever, right-click a living entity, right-click the armor stand's arm while holding the block stack (the armor-stand equip path — a plain block is accepted only into a hand slot, which is why the stand needs `ShowArms:1b`), press the off-hand swap key, drop the held stack, walk over the dropped item; then run the same command again as an after-control. Read `data get entity <player> SelectedItem`, `data get entity <player> Inventory`, and the armor stand's full entity data before and after | Neither control command is accepted, which proves the player stayed unauthenticated for the whole sequence. None of the eight actions takes effect: the target block is still present and no new block appears, the lever keeps its previous state, `SelectedItem` still reads count 5 (in the reported defect it fell to 4), the armor stand's entity data shows no item in any armor or hand slot, the off hand is still empty and the selected slot unchanged, no dropped-item entity appears for the drop, and the item already on the ground is still an entity afterwards | server | |
