...
- **Kind**, exactly these eight values: `command`, `config`, `event`, `gui`, `scheduled`,
  ...
  This module was the first to carry `gui`-Kind rows: its two GUI classes excluded from the
  coverage gate (`LoginGUIPage`, `RegisterGUIPage`) are catalogued as their own rows, distinct
  from the
...
3. **Javadoc and string literals** — requiring the annotation to start its own line (the
   ...
   reference method names like `#invalidateSession(UUID)` and review threads extensively, but
...
(`grep -c '@EventListener'` confirms this: exactly 2), registering 27 `@EventHandler` methods between
them (23 and 4), all at `EventPriority.LOWEST` except `#onPlayerQuit`. This section groups the 27
...
`FEATURES.md` (`ChatListener#onChat`) already apply — ten of the twenty-seven handlers
...
adds a null guard, so neither is byte-identical — and
...
`@EventHandler` line (27) and this section's row count (8) are reconciled by this paragraph, not
...
`ultilogin.protection.inventory-block`, whose Feature cells name them. UltiKits/UltiLogin#41 raised it
to 27, both into `ultilogin.protection.movement-block` (`onEntityMount`, `onVehicleMove`).
...
| ultilogin.protection.movement-block | Hold an unauthenticated player in place. Walking to another block is reverted (looking around in place is deliberately allowed — only a change to `getBlockX`/`Y`/`Z` is reverted), and so is steering a vehicle: Paper fires the same `PlayerMoveEvent` for the rider who controls a vehicle, and reverting it makes the server teleport the rider back, which dismounts them (measured from the server's bytecode, UltiKits/UltiLogin#33). A passenger who does not steer gets no such event (read from the server's bytecode) and was carried by the vehicle (measured on a real server), so an unauthenticated player rides nothing (UltiKits/UltiLogin#41): a mount by an unauthenticated player is refused (`#onEntityMount`), except the vehicle the server restores in the join tick for a player who quit while riding — refusing that one makes the server delete the vehicle and everything riding it — so that player is taken off it one tick after joining and the vehicle stays where it is (`#onPlayerJoin`); a player whose credentials are revoked while riding is dismounted with the other no-session protections (`LoginService#applyNoSessionProtections`); and a moving minecart or boat drops an unauthenticated passenger (`#onVehicleMove`). A player who quit while riding therefore finds the vehicle beside them, not under them, when they log in | event | attempt to move or ride while not logged in, or join while riding | n/a | n/a | player | detailed | LoginProtectionListener#onPlayerMove, LoginProtectionListener#onEntityMount, LoginProtectionListener#onVehicleMove, LoginProtectionListener#onPlayerJoin, LoginService#applyNoSessionProtections |
