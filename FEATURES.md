...
|---|---|---|---|---|---|---|---|---|
| ultilogin.login.authenticate | Authenticate a registered, currently-unauthenticated player against their stored password hash; locks the account/IP after too many wrong attempts (see `## Player Protection`'s security-lockout config rows). A wrong password is answered with the attempts remaining, with `messages.account-locked` on the attempt that reaches `security.max-login-attempts`, or — when that setting is `0` (unlimited), where nothing is ever locked — with the language catalogue's `wrong_password` text (UltiKits/UltiLogin#23) | command | `/login <password>` (alias `/l`) | none | player | player | brief | LoginCommand#login |
