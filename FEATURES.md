...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultilogin.lifecycle.legacy-text-defaults | When the module is enabled and again on every reload of it, after the framework has read `config/login.yml`: each of the three `gui-mode.title-*` values and the twenty-one `messages.*` values that is exactly the Chinese default an earlier version shipped is replaced with a blank value and the file is saved, so those texts come from the language file in the server's language; a value that differs in any way (an operator's edit) is kept, and a blank value is never rewritten again, so the file is saved only when something changed. A failed save is logged as one warning from the language file (`log_config_default_save_failed`) and costs nothing else (UltiKits/UltiLogin#20) | event | module enable and every reload of it: bare `/ul reload`, which reloads every module, or `/ul reload UltiLogin` | n/a | n/a | admin | brief | UltiLogin#registerSelf, UltiLogin#onReload, LoginConfig#migrateLegacyDefaults |
...
| ultilogin.config.login.gui-mode.title-confirm | GUI title shown during the register page's password-confirmation phase. Blank shows the language file's `gui_confirm_password`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: gui-mode.title-confirm (default: blank)` | n/a | n/a | admin | none | RegisterGUIPage#processInput |
| ultilogin.config.login.gui-mode.title-login | GUI title shown on `LoginGUIPage`. Blank shows the language file's `gui_enter_password`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: gui-mode.title-login (default: blank)` | n/a | n/a | admin | none | LoginGUIPage#LoginGUIPage |
| ultilogin.config.login.gui-mode.title-register | GUI title shown on `RegisterGUIPage`'s first phase. Blank shows the language file's `gui_set_password`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: gui-mode.title-register (default: blank)` | n/a | n/a | admin | none | RegisterGUIPage#RegisterGUIPage |
...
| ultilogin.config.login.messages.account-locked | Message shown for a locked-out login attempt, with `{TIME}` substituted. Blank shows the language file's `account_locked`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.account-locked (default: blank)` | n/a | n/a | admin | brief | LoginCommand#login |
| ultilogin.config.login.messages.admin.account-not-found | Message shown to an admin naming a player with no account, with `{PLAYER}` substituted. Blank shows the language file's `admin_account_not_found`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.admin.account-not-found (default: blank)` | n/a | n/a | admin | none | LoginAdminCommand#resetPassword |
| ultilogin.config.login.messages.admin.force-login | Message shown to an admin on a successful `/logadmin forcelogin`, with `{PLAYER}` substituted. Blank shows the language file's `admin_force_login`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.admin.force-login (default: blank)` | n/a | n/a | admin | none | LoginAdminCommand#forceLogin |
| ultilogin.config.login.messages.admin.password-reset | Message shown to an admin on a successful `/logadmin reset`, with `{PLAYER}`/`{PASSWORD}` substituted. Blank shows the language file's `admin_password_reset`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.admin.password-reset (default: blank)` | n/a | n/a | admin | none | LoginAdminCommand#resetPassword |
| ultilogin.config.login.messages.admin.player-not-found | Message shown to an admin naming a player who is not online, with `{PLAYER}` substituted. Blank shows the language file's `admin_player_not_found`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.admin.player-not-found (default: blank)` | n/a | n/a | admin | none | LoginAdminCommand#forceLogin |
| ultilogin.config.login.messages.admin.unregister | Message shown to an admin on a successful `/logadmin unregister`, with `{PLAYER}` substituted. Blank shows the language file's `admin_unregister`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.admin.unregister (default: blank)` | n/a | n/a | admin | none | LoginAdminCommand#unregister |
| ultilogin.config.login.messages.already-logged | Message shown when a login/register is attempted while already logged in. Blank shows the language file's `already_logged`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.already-logged (default: blank)` | n/a | n/a | admin | brief | LoginCommand#login |
| ultilogin.config.login.messages.already-registered | Message shown when `/register` is run by an already-registered player. Blank shows the language file's `already_registered`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.already-registered (default: blank)` | n/a | n/a | admin | brief | RegisterCommand#register |
| ultilogin.config.login.messages.attempts-remaining | Message shown after a wrong password, with `{COUNT}` substituted with attempts remaining before lockout. Blank shows the language file's `attempts_remaining`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.attempts-remaining (default: blank)` | n/a | n/a | admin | brief | LoginCommand#login |
| ultilogin.config.login.messages.gui-password-invalid | Message shown for a GUI-mode password rejected by validation, with `{LENGTH}` substituted. Blank shows the language file's `gui_password_invalid`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.gui-password-invalid (default: blank)` | n/a | n/a | admin | none | LoginService#getPasswordValidationError |
| ultilogin.config.login.messages.login-prompt | Text-mode login prompt shown on join/blocked-action for a registered, unauthenticated player. Blank shows the language file's `login_prompt`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.login-prompt (default: blank)` | n/a | n/a | admin | brief | LoginService#onPlayerJoin |
| ultilogin.config.login.messages.login-prompt-gui | GUI-mode login prompt text shown on the same trigger as `messages.login-prompt`, before the GUI itself opens. Blank shows the language file's `login_prompt_gui`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.login-prompt-gui (default: blank)` | n/a | n/a | admin | none | LoginService#onPlayerJoin |
| ultilogin.config.login.messages.login-success | Message shown on successful `/login`. Blank shows the language file's `login_success`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.login-success (default: blank)` | n/a | n/a | admin | brief | LoginCommand#login |
| ultilogin.config.login.messages.not-registered | Message shown when `/login` is run by an unregistered player. Blank shows the language file's `not_registered`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.not-registered (default: blank)` | n/a | n/a | admin | brief | LoginCommand#login |
| ultilogin.config.login.messages.password-mismatch | Message shown when `/register`'s two password entries do not match; also read by `RegisterGUIPage#processInput` when the GUI-mode confirmation phase does not match the first phase. `/logadmin reset` has no confirmation parameter at all (its two mappings take either one generated password or one admin-supplied password) and never reads this key. Blank shows the language file's `password_mismatch`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.password-mismatch (default: blank)` | n/a | n/a | admin | brief | RegisterCommand#register, RegisterGUIPage#processInput |
| ultilogin.config.login.messages.password-too-long | Message shown for a command-mode password exceeding `password.max-length`, with `{MAX}` substituted. Blank shows the language file's `password_too_long`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.password-too-long (default: blank)` | n/a | n/a | admin | none | LoginService#getPasswordValidationError |
| ultilogin.config.login.messages.password-too-short | Message shown for a command-mode password below `password.min-length`, with `{MIN}` substituted. Blank shows the language file's `password_too_short`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.password-too-short (default: blank)` | n/a | n/a | admin | none | LoginService#getPasswordValidationError |
| ultilogin.config.login.messages.register-prompt | Text-mode registration prompt shown on join/blocked-action for an unregistered player. Blank shows the language file's `register_prompt`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.register-prompt (default: blank)` | n/a | n/a | admin | brief | LoginService#onPlayerJoin |
| ultilogin.config.login.messages.register-prompt-gui | GUI-mode registration prompt text shown on the same trigger as `messages.register-prompt`. Blank shows the language file's `register_prompt_gui`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.register-prompt-gui (default: blank)` | n/a | n/a | admin | none | LoginService#onPlayerJoin |
| ultilogin.config.login.messages.register-success | Message shown on successful `/register` (both text and GUI mode). Blank shows the language file's `register_success`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.register-success (default: blank)` | n/a | n/a | admin | brief | RegisterCommand#register |
| ultilogin.config.login.messages.timeout-kick | Kick message shown to a player removed by `ultilogin.task.timeout-check`. Blank shows the language file's `timeout_kick`, so it follows `language`; a non-blank value is shown as written (UltiKits/UltiLogin#20) | config | `config/login.yml: messages.timeout-kick (default: blank)` | n/a | n/a | admin | brief | LoginService#checkTimeouts |
...
messages of `config/login.yml` are operator-editable: a blank value, the default since this
change, shows the language file's text, and any other value is shown as written
