...
| ultilogin.config.login.messages.wrong-password | Message shown for an incorrect `/login` password when the account is not yet at the lockout threshold | config | `config/login.yml: messages.wrong-password (default: Simplified Chinese text, not reproduced per D-02 -- see this same file's source line for the exact characters)` | n/a | n/a | admin | none | LoginService#login |
