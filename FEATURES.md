...
| ultilogin.protection.movement-block | Hold an unauthenticated player in place. Walking to another block is reverted (looking around in place is deliberately allowed — only a change to `getBlockX`/`Y`/`Z` is reverted), and so is steering a vehicle: Paper fires the same `PlayerMoveEvent` for the rider who controls a vehicle, and reverting it makes the server teleport the rider back, which dismounts them (measured from the server's bytecode, UltiKits/UltiLogin#33). A passenger who does not steer gets no such event and would be carried by the vehicle, so an unauthenticated player rides nothing (UltiKits/UltiLogin#41, measured on a real server): a mount by an unauthenticated player is refused, including the vehicle the server restores on join for a player who quit while riding (`#onEntityMount`); a player still riding one tick after joining and not logged in is dismounted (`#onPlayerJoin`); a player whose credentials are revoked while riding is dismounted with the other no-session protections (`LoginService#applyNoSessionProtections`); and a moving minecart or boat drops an unauthenticated passenger (`#onVehicleMove`). A player who quit while riding is therefore not in the vehicle when they log in | event | attempt to move or ride while not logged in, or join while riding | n/a | n/a | player | detailed | LoginProtectionListener#onPlayerMove, LoginProtectionListener#onEntityMount, LoginProtectionListener#onVehicleMove, LoginProtectionListener#onPlayerJoin, LoginService#applyNoSessionProtections |
